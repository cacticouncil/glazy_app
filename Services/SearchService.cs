...
namespace ASTEM_DB.Services
{
    ...
    public class SearchService
    {
        ...
        public async Task<List<SearchMatch>> SearchByImageAsync(string imagePath, CancellationToken cancellationToken)
        {
            ...
            using var response = await _httpClient.PostAsync($"{ApiBase}/search/image", content, cancellationToken);
            ...
            {
                var tileId = match.TryGetProperty("tile_id", out var tileIdElement)
                    ? tileIdElement.GetString()
                    : string.Empty;
